...
namespace App\Http\Controllers\Kasus\Laparoscopic;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Models\Kasus;
use App\Exports\LaparoscopicExcel;
use Auth;
...
class ViewController extends Controller
{
	...
	{
		$user_id = Auth::user()->id;
		$result = Kasus::where('jenis_kasus','laparoscopic')->with('pasien','penunjang_radiology')->where('created_by',$user_id)->skip($start)->take($limit)->get();


		$data['kasus'] = $result;

		$data['view'] = 'kasus.laparoscopic.print';
		return (new LaparoscopicExcel($data))->download('Laparoscopic.xlsx');
	}
}
